...
# Set up the plot with a rectangular shape
fig, ax = plt.subplots(figsize=(15, 8))

# Plot the line graph for the filtered data with adjusted line width
for country in countries:
    country_data = filtered_data[filtered_data['Country Name'] == country]
    ax.plot(country_data['Year'], country_data['Value'], marker='o', linewidth=2.5, label=country)

# Set plot title, labels, and legend to the top right corner
...
fig, ax2 = plt.subplots(figsize=(2, 3), gridspec_kw={'top': 0.8, 'bottom': 0.1})

# Create a heatmap
heatmap_data = df.pivot(index='Country Name', columns='Year', values='Value')
sns.heatmap(heatmap_data, cmap='coolwarm', ax=ax2, cbar_kws={'label': 'Value'})
